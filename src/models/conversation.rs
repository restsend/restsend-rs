...
impl DBStore {
    pub fn get_conversations_count(&self) -> crate::Result<u32> {
    ...
    pub fn get_conversation(&self, topic_id: &str) -> crate::Result<Conversation> {
        let conn = self.pool.get()?;
    ...
    pub fn remove_conversation(&self, topic_id: &str) -> crate::Result<()> {
        let conn = self.pool.get()?;
        conn.execute(
    ...
    pub fn set_conversation_sticky(&self, topic_id: &str, sticky: bool) -> crate::Result<()> {
        let conn = self.pool.get()?;
        conn.execute(
    ...
    pub fn set_conversation_mute(&self, topic_id: &str, mute: bool) -> crate::Result<()> {
        let conn = self.pool.get()?;
        conn.execute(
