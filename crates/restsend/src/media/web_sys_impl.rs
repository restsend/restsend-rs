...
pub async fn upload_file(
    ...
    let blob_stream = web_sys::Blob::new_with_u8_array_sequence_and_options(
        &JsValue::from_bool(true),
        web_sys::BlobPropertyBag::new().type_("application/octet-stream"),
    )?;

    let total = blob_stream.size() as u64;
    ...
    let upload_runner = async move {
        ...
        let data_stream = JsFuture::from(blob_stream.array_buffer()).await?;
