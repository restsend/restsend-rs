...
impl callback::RsCallback for DemoWsCallback {
    ...
    fn on_conversations_updated(&self, conversations: Vec<Conversation>, _total: Option<i64>) {
        for conv in &conversations {
            self.conv_unreads.lock().unwrap().push((conv.topic_id.clone(), conv.unread));
        }
    }
}
...
async fn test_demo_dm_alice_bob_websocket_delivery() {
    ...
    // login
    let info_alice = login_with_password(endpoint.clone(), "alice".to_string(), DEMO_PASS.to_string())
        .await
        .expect("login alice");
    ...
    assert_eq!(conv.topic_id, "alice:bob", "DM topic ID should be 'alice:bob'");
    ...
    client_alice
        .do_send_text(
            ...
            None,
            None,
            Some(Box::new(AckCallback { acked: acked.clone() })),
        )
        .await
    ...
    check_until(WS_TIMEOUT, || {
        bob_received_topic_id.lock().unwrap().contains(&"alice:bob".to_string())
    })
    .await
    ...
    check_until(WS_TIMEOUT, || {
        alice_received_topic_id.lock().unwrap().contains(&"alice:bob".to_string())
    })
    .await
    ...
    let bob_topic_unreads: Vec<i64> = bob_unreads.iter()
    ...
    let bob_second_received = check_until(Duration::from_secs(3), || {
        let count = bob_received_topic_id.lock().unwrap().iter()
            .filter(|t| *t == "alice:bob").count();
        count >= 2
    }).await;
    assert!(bob_second_received.is_ok(), "bob should receive second msg via ws");

    // Bob's unread should now be >= 2 (hasRead=false)
    tokio::time::sleep(Duration::from_millis(500)).await;
    let bob_unreads_2 = bob_conv_unreads.lock().unwrap().clone();
    let bob_topic_unreads_2: Vec<i64> = bob_unreads_2.iter()
        .filter(|(t, _)| t == "alice:bob")
        .map(|(_, u)| *u)
        .collect();
    ...
    assert!(
        ...
        max_unread, bob_topic_unreads_2
    );
    log::info!("=== Unread count verified: bob's unread = {} ===", max_unread);
    ...
    client_bob
        .do_send_text(
            ...
            None,
            None,
            Some(Box::new(AckCallback { acked: bob_acked.clone() })),
        )
        .await
    ...
    let alice_msg_count = alice_received_topic_id.lock().unwrap().iter().filter(|t| *t == "alice:bob").count();
    assert!(
        alice_msg_count >= 2,
    ...
    client_bob.set_conversation_read(conv.topic_id.clone(), false).await;
    tokio::time::sleep(Duration::from_millis(500)).await;
    ...
    log::info!(
        ...
        bob_conv_after_read.unread, bob_conv_after_read.last_read_seq, bob_conv_after_read.last_seq
    );
    ...
    struct SyncDone { done: std::sync::atomic::AtomicBool, count: std::sync::atomic::AtomicU32 }
    ...
    client_bob
        .sync_conversations(None, None, None, None, 0, false, None, None, None, Box::new(SyncDone {
    ...
    log::info!(
        ...
        bob_conv_after_third.last_seq, bob_conv_after_third.unread, bob_conv_after_third.last_sender_id
    );
