...
impl Client {
    ...
    pub async fn get_topic_admins(&self, topic_id: &str) -> Option<Vec<User>> {
    ...
    pub async fn get_topic_owner(&self, topic_id: &str) -> Option<User> {
        match self.get_topic(topic_id).await {
    ...
    pub async fn get_topic_knocks(&self, topic_id: &str) -> Option<Vec<TopicKnock>> {
    ...
    pub async fn update_topic_notice(&self, topic_id: &str, text: &str) -> Result<()> {
        update_topic_notice(&self.endpoint, &self.token, topic_id, text).await
    }

    pub async fn silent_topic(&self, topic_id: &str, duration: Option<String>) -> Result<()> {
    ...
    pub async fn add_topic_admin(&self, topic_id: &str, user_id: &str) -> Result<()> {
        add_topic_admin(&self.endpoint, &self.token, topic_id, user_id).await
    }

    pub async fn remove_topic_admin(&self, topic_id: &str, user_id: &str) -> Result<()> {
        remove_topic_admin(&self.endpoint, &self.token, topic_id, user_id).await
    }

    pub async fn transfer_topic(&self, topic_id: &str, user_id: &str) -> Result<()> {
        transfer_topic(&self.endpoint, &self.token, topic_id, user_id).await
    }

    pub async fn quit_topic(&self, topic_id: &str) -> Result<()> {
        quit_topic(&self.endpoint, &self.token, topic_id).await
    }

    pub async fn dismiss_topic(&self, topic_id: &str) -> Result<()> {
        dismiss_topic(&self.endpoint, &self.token, topic_id).await
    }

    pub async fn accept_topic_join(&self, topic_id: &str, user_id: &str, memo: &str) -> Result<()> {
    ...
    pub async fn remove_topic_member(&self, topic_id: &str, user_id: &str) -> Result<()> {
